...
const formatMovieData = (tmdbMovie) => {
  return {
    ...
    Production:
      ...
      tmdbMovie.production_companies.length > 0
...
export const searchMovies = async (query) => {
  ...
  try {
    ...
    if (!query || query.trim() === "") {
      ...
      url = `${TMDB_CONFIG.baseUrl}/search/movie?api_key=${
        TMDB_CONFIG.apiKey
      }&query=${encodeURIComponent(query)}&with_original_language=ta`;
    }
    ...
    for (const movie of moviesToProcess) {
      try {
        ...
        if (details.credits) {
          ...
          const mainActors = details.credits.cast
            .slice(0, 3)
